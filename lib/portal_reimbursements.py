...
PORTAL_TRACKINGS_FILENAME = "portal_trackings.pickle"
PORTAL_POS_FILENAME = "portal_pos.pickle"
...
class PortalReimbursements:

  def __init__(self, config):
    self.retriever = ObjectRetriever(config)
    self.trackings_to_costs: Dict[Tuple[str],
                                  Tuple[str,
                                        float]] = self.retriever.load(PORTAL_TRACKINGS_FILENAME)
    self.po_to_cost: Dict[str, float] = self.retriever.load(PORTAL_POS_FILENAME)

  def flush(self):
    self.retriever.flush(self.trackings_to_costs, PORTAL_TRACKINGS_FILENAME)
    self.retriever.flush(self.po_to_cost, PORTAL_POS_FILENAME)
